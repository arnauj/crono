  ...
  return (
    <TimerLayout title={t('mode.amrap')} subtitle={subtitle} phase={state.phase} onBack={handleBack}>
      {state.phase === 'idle' ? (
        ...
        <>
          <TimerDisplay time={state.timeLeft} phase={state.phase} onClick={state.phase !== 'done' ? pause : undefined} typeLabel={t('mode.amrap')} elapsed={state.elapsed} isRunning={state.isRunning} />
